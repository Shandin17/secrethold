...
class SecretHold {
  ...
  async changePin({ oldPin, newPin, id }, tx = null) {
    ...
    let newEncryptedKey;
    try {
      decryptedKey = await decrypt(encryptedKey, this.#masterKey, oldPin, this.secretEncoding);
      if (this.secretWrapper !== null) await this.secretWrapper(decryptedKey);
      const newPinSalt = randomBytes(saltLength);
      newEncryptedKey = await encrypt(
        decryptedKey,
        this.#masterKey,
        newPin,
        newPinSalt,
