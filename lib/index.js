...
class KeyKeeper {
    ...
    if (!decryptedSecret) {
      ...
      if (encryptedSecret) {
        try {
          ...
          throw new Error(`KeyKeeper error: wrong pin provided`, {
            code: WRONG_PIN,
          });
        }
      }
  ...
  async changePin({ oldPin, newPin, userId }) {
    ...
    if (!encryptedKey) {
      throw new Error(`KeyKeeper error: unknown userId provided.`, {
        code: WRONG_USER_ID,
      });
    }
    let decryptedKey;
  ...
  async setSecret({ userId, decryptedSecret, pin }, tx = null) {
    ...
    const encryptedSecret = await encrypt(decryptedSecret, this.#masterKey, pin, pinSalt);
    await Promise.all([
