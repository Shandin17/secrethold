...
class Secrethold {
  ...
  async createEncryptionStream({ source, pin }) {
    ...
    const iv = randomBytes(saltLength);
    ...
    return {
      pinSalt: pinSalt.toString(this.#encryptedDataEncoding),
      iv: iv.toString(this.#encryptedDataEncoding),
      encryptedStream,
      masterTagPromise: masterTagPromise.then((tag) => tag.toString(this.#encryptedDataEncoding)),
      pinTagPromise: pinTagPromise.then((tag) => tag.toString(this.#encryptedDataEncoding)),
    };
  }
  ...
  async createDecryptionStream({ encryptedSource, pin, pinSalt, iv, masterTag, pinTag }) {
    const decryptStream = await decrypt({
      ...
      encryptedSource,
      pinSalt:
        pinSalt instanceof Buffer ? pinSalt : Buffer.from(pinSalt, this.#encryptedDataEncoding),
      iv: iv instanceof Buffer ? iv : Buffer.from(iv, this.#encryptedDataEncoding),
      masterTag:
        masterTag instanceof Buffer
          ? masterTag
          : Buffer.from(masterTag, this.#encryptedDataEncoding),
      pinTag: pinTag instanceof Buffer ? pinTag : Buffer.from(pinTag, this.#encryptedDataEncoding),
    });
