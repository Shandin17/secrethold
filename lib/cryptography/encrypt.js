...
const { keeperCryptoAlgorithm, saltLength } = require('./constants.js');
...
async function encrypt(data, masterKey, pin, pinSalt, secretsEncoding) {
  ...
  const cipherMaster = crypto.createCipheriv(keeperCryptoAlgorithm, masterKey, ivMaster);
  ...
  const cipher = crypto.createCipheriv(keeperCryptoAlgorithm, keyFromPin, iv);
